...
- **Selection Transform Commands**: Flip horizontal (`Shift H`), flip vertical (`Shift V`), rotate (`Shift R`), and scale (`Shift S`) operate on any selection through per-primitive operators; rotate and scale open a dialog with an artboard-center or selection-bounding-box pivot whose choice is remembered per command.
